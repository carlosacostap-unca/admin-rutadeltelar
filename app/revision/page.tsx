...
export default function RevisionPage() {
  ...
  useEffect(() => {
    async function fetchRevisionItems() {
      if (!user) return;
      try {
        setLoading(true);
    ...
    <div className="h-full bg-[var(--color-surface-dim)]">
      <main className="mx-auto px-6 py-8">
        ...
        <div className="flex flex-col gap-8">
          {loading ? (
            <p className="p-8 text-center text-[var(--color-on-surface-variant)]">Cargando elementos pendientes...</p>
          ) : items.length === 0 ? (
            <div className="bg-[var(--color-surface-container)] p-8 text-center text-[var(--color-on-surface-variant)] rounded-md">
              No hay elementos pendientes de revisión. ¡Todo está al día!
            </div>
          ) : (
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    {colItems.map((item) => (
                      <Link 
                        ...
                        className="bg-[var(--color-surface-container)] p-5 rounded-xl hover:bg-[var(--color-surface-container-low)] transition-all shadow-sm flex flex-col gap-2 cursor-pointer"
                      >
                        <div className="flex justify-between items-start">
                          <h4 className="text-md font-bold text-[var(--color-primary)]">{item.title}</h4>
                          <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-[0.05em] shrink-0 bg-[var(--color-surface-variant)] text-[var(--color-on-surface)]">
                            EN REVISIÓN
                          </span>
                        </div>
